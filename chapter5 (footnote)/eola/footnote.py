...
class QuickFootnote(TeacherStudentsScene):
    def construct(self):
        self.dither(0.1)
        self.setup()
        self.teacher_says(TextMobject(u"\\textarmenian{Փոքրիկ ծանոթագրություն}"))
        self.random_blink()
        self.play(
...
class Testik(Scene):
    def construct(self):
        self.play(Write(TexMobject("A")))

class PeakOutsideFlatland(TeacherStudentsScene):
    def construct(self):
        self.dither(0.1)
        self.setup()
        self.teacher_says(TextMobject(u"\\textarmenian{Նայեք հարթությունից դուրս}"))
        self.dither()
...
class SymbolicThreeDTransform(Scene):
    CONFIG = {
        "input_coords" : [2, 6, -1],
        "output_coords" : [3, 2, 0],
        "title" : u"\\textarmenian{Եռաչափ ձևափոխություն}",
    }
    def construct(self):
        ...
        in_words = TextMobject(u"\\textarmenian{Մուտք}")
        in_words.next_to(in_vect, DOWN)
        in_words.highlight(BLUE_C)
        out_words = TextMobject(u"\\textarmenian{Ելք}")
...
class InputWordOutputWord(Scene):
    def construct(self):
        self.dither(0.1)
        text1 = TextMobject(u"\\textarmenian{Համակարգչային գրաֆիկա}")
        text2 = TextMobject(u"\\textarmenian{Ռոբոտաշինություն}")
        text1.next_to(text2,UP,buff=0.9)
        text2.next_to(text1,DOWN,buff=1.3)
        self.play(Write(text1.scale(1.3)))
        self.play(Write(text2.scale(1.3)))
...
class ShowMatrixVectorMultiplication(Scene):
    def construct(self):
        ...
        trips = [
            (matrix, DOWN, u"\\textarmenian{Ձևափոխություն}"),
            (vect, UP, u"\\textarmenian{Մուտքային վեկտոր}"),
            (result, DOWN, u"\\textarmenian{Ելքային վեկտոր}"),
        ]
        braces = []
...
class ShowMatrixMultiplication(Scene):
    def construct(self):
        ...
        trips = [
            (right, DOWN, u"\\textarmenian{Առաջին ձևափոխություն}"),
            (left, UP, u"\\textarmenian{Երկրորդ ձևափոխություն}"),
        ]
        braces = []
        for mob, direction, text in trips:
            brace = Brace(mob, direction)
...
class NextVideo(Scene):
    def construct(self):
        title = TextMobject(u"\\textarmenian{Հաջորդ տեսանյութը․ «Որոշիչ»}")
