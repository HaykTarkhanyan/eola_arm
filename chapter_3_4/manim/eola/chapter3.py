...
class OpeningQuote(Scene):
    def construct(self):
        ...
        author = TextMobject(u"\\textarmenian{— Մորֆեուս}")
        author.highlight(YELLOW)
        author.next_to(words, DOWN, buff = 0.5)
        comment = TextMobject(u"""\\textarmenian{
            (Զարմանալիորեն դիպուկ խոսքեր մատրիցներով գործողությունների
              տեսողական ընկալման կարևորության մասին)}""")
...
class MatrixVectorMultiplication(LinearTransformationScene):
    ...
    def label_matrix(self, matrix):
        title = TextMobject(u"\\textarmenian{«}2x2 \\textarmenian{մատրից»}")
        ...
        i_message = TextMobject(u"\\textarmenian{Որտեղ է գնում $\\hat{\\imath}$-ն}")
        j_message = TextMobject(u"\\textarmenian{Որտեղ է գնում $\\hat{\\jmath}$-ն}")
...
class OtherWayAround(Scene):
    def construct(self):
        self.play(Write(TextMobject(u"\\textarmenian{Իսկ ի՞նչ, եթե հակառակն ենք անում}")))
        self.dither(2)
