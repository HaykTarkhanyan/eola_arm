...
class OpeningQuote(Scene):
    def construct(self):
        words = TextMobject([
            u"\\textarmenian{Իմ փորձից կարող եմ ասել,որ,}",
            u"\\textarmenian{մատրիցների}",
            u"\\textarmenian{միջոցով կատարվող ապացույցները 50\\%-ով ավելի սեղմ կլինեն եթե}",
            u"\\textarmenian{մատրիցներից ազատվենք:}"
        ])
        words.scale_to_fit_width(2*SPACE_WIDTH - 2)
        words.to_edge(UP)
        words.split()[1].highlight(GREEN)
        words.split()[3].highlight(BLUE)
        author = TextMobject(u"\\textarmenian{-Էմիլ Արթին}")
...
class RecapTime(TeacherStudentsScene):
    def construct(self):
        self.setup()
        self.teacher_says(TextMobject(u"\\textarmenian{Արագ ամփոփե՜նք}"))
        self.random_blink()
        self.dither()
...
class DeterminedByTwoBasisVectors(LinearTransformationScene):
    ...
    def construct(self):
        self.setup()
        i_hat = self.add_vector([1.0, 0.0], color = X_COLOR)
        self.add_transformable_label(
            i_hat, "\\hat{\\imath}", "\\hat{\\imath}", 
            color = X_COLOR
        )
        j_hat = self.add_vector([0.0, 1.0], color = Y_COLOR)
        self.add_transformable_label(
...
class RecapOver(TeacherStudentsScene):
    def construct(self):
        self.setup()
        self.teacher_says(TextMobject(u"\\textarmenian{ամփոփոմը ավարտվեց}"))
...
class RotationThenShear(LinearTransformationScene):
    ...
    def construct(self):
        self.setup()
        rot_words = TextMobject(u"\\textarmenian{$90^\\circ$ պտույտ ժամսլաքին հակառակ}")
        shear_words = TextMobject(u"\\textarmenian{տեքստ ինչ որ}")
...
class IntroduceIdeaOfComposition(RotationThenShear):
    ...
    def show_composition(self):
        words = TextMobject([
            u"\\textarmenian{Պտույտի}",
            u"\\textarmenian{եւ}",
            u"\\textarmenian{ձգման}",
            u"\\textarmenian{«Համադրույթ»}"

        ])
        words.split()[3].submobject_gradient_highlight(YELLOW, PINK, use_color_range_to = False)
        words.split()[0].highlight(YELLOW)
        words.split()[2].highlight(PINK)
        words.add_background_rectangle()
        words.to_edge(UP)
