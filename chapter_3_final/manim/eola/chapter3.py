...
class OpeningQuote(Scene):
    def construct(self):
        words = TextMobject([
            u"\\textarmenian{Ցավոք, որևէ մեկի հնարավոր չէ բացատրել, թե ինչ է }",
            u"\\textarmenian{Մատրիցը։}",
            u"\\textarmenian{Այն}",
            u"\\textarmenian{ինքդ պետք է տեսնես։}",
        ])
        words.scale_to_fit_width(2*SPACE_WIDTH - 2)
        words.to_edge(UP)
        words.split()[1].highlight(GREEN)
        words.split()[3].highlight(BLUE)
        author = TextMobject(u"\\textarmenian{— Մորֆիուս}")
        author.highlight(YELLOW)
        author.next_to(words, DOWN, buff = 0.5)
        comment = TextMobject(u"""\\textarmenian{
            (Զարմանալիորեն դիպուկ խոսքեր մատրիցներով գործողությունների տեսողական ընկալման \\\\կարևորության մասին)}""").scale(0.85)
...
class Introduction(TeacherStudentsScene):
    def construct(self): # Մատրիցները որպես գծային ձևափոխություն
        title = TextMobject([u"\\textarmenian{Մատրիցները որպես}", u"\\textarmenian{գծային ձևափոխություն}"])
        title.to_edge(UP)
        title.highlight(YELLOW)
        linear_transformations = title.split()[1]
        self.add(*title.split())
        self.setup()
        t = TextMobject(u"""\\textarmenian{
            Ուշադի՛ր, ընկերներ, \\\\ սա հատկապես կարևոր է}
        """)#.scale(4) chi stacvum mecacnel
        
        self.teacher_says(t, height = 3)
        self.random_blink(2)
        self.teacher_thinks("", height = 3)
        self.remove(linear_transformations)

        self.play(Transform(linear_transformations, TextMobject(u"\\textarmenian{Գծային ձևափոխություն}").to_edge(UP).highlight(YELLOW)), runtime=0.01)
...
class DescribeTransformation(Scene):
    ...
    def add_title(self):
        title = TextMobject([u"\\textarmenian{Գծային}", u"\\textarmenian{ձևափոխություն}"])
        title.to_edge(UP)
        linear, transformation = title.split()
        brace = Brace(transformation, DOWN)
        function = TextMobject(u"\\textarmenian{ֆունկցիա}").next_to(brace, DOWN)
    ...
    def show_function(self):
        ...
        vector_input_words = TextMobject(u"   \\textarmenian{սկզբնական} \\\\ \\textarmenian{(մուտքի) վեկտոր}")
        vector_input_words.highlight(MAROON_C)
        vector_input_words.next_to(input_vect, DOWN) 
        vector_output_words = TextMobject(u"  \\textarmenian{ստացված} \\\\ \\textarmenian{(ելքի) վեկտոր}")
...
class WhyConfuseWithTerminology(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.student_says(TextMobject(u"\\textarmenian{Ինչո՞ւ եք մեզ շփոթեցնում \\\\ ավելորդ տերմինաբանությամբ}"))
        ...
        statement = TextMobject([
            u"\\textarmenian{«Ձևափոխություն»}",
            u"\\textarmenian{բառը հուշում է, որ \\\\խոսքն այստեղ}",
            u"\\textarmenian{շարժման}",
            u"\\textarmenian{մասին է}"
        ])
#  բառը հուշում է, որ խոսքն այստեղ շարժման մասին է


        statement.split()[0].highlight(BLUE)
        statement.split()[-2].highlight(YELLOW)
...
class LookToWordLinear(Scene):
    def construct(self):  # karoxa dzevapoxutun vochte dzevapoxutunner?
        title = TextMobject([u"\\textarmenian{Գծային} ", u"\\textarmenian{ձևափոխություններ}"])
        title.to_edge(UP)
        ...
        function = TextMobject(u"\\textarmenian{ֆունկցիա}")
        function.highlight(YELLOW)
        function.next_to(transformation_brace, DOWN)
        new_sub_word = TextMobject(u"\\textarmenian{Ի՞նչ է սա նշանակում}")
...
class IntroduceLinearTransformations(LinearTransformationScene):
    ...
    def construct(self):
        ...
        lines_rule = TextMobject(u"\\textarmenian{Ուղիղները ուղիղ են մնում}").scale(0.9) # menq avelacrecinq scaley
        lines_rule.shift(3*UP).to_edge(LEFT) # 2 angam er, darav 3
        origin_rule = TextMobject(u"\\textarmenian{Սկզբնակետը չի տեղաշարժվում}").scale(0.9)
        origin_rule.shift(2*UP).to_edge(RIGHT) # angam 2y menq enq avelacrel
...
class SimpleNonlinearTransformationScene(LinearTransformationScene):
    CONFIG = {
        "show_basis_vectors" : False,
        "words" : u"\\textarmenian{Ոչ գծային․ որոշ ուղիղներ կորանում են}"
    }
    def construct(self):
        self.setup()
        self.dither()
        ...
        words.to_corner(UP) # + RIGHT kar, jnjecinq
        words.highlight(RED)
...
class MovingOrigin(SimpleNonlinearTransformationScene):
    # 02_56 - 03_02 MovingOrigin
    CONFIG = {
        "words" : u"\\textarmenian{Ոչ գծային. սկզբնակետը տեղաշարժվում է}"
    }
    def setup(self):
...
class SneakyNonlinearTransformationExplained(SneakyNonlinearTransformation):
    # 03_11 - 03_16 
    CONFIG = {
        "words" : u"\\textarmenian{Ոչ գծային․ անկյունագծային ուղիղները կորանում են}"
    }
    def setup(self):
        LinearTransformationScene.setup(self)
        diag = Line(
...
class GridLinesRemainParallel(SimpleLinearTransformationScene):
    ...
    def construct(self):
        ...
        text = TextMobject([ # Կոորդինատային ցանցի գծերը մնում են զուգահեռ և միմյանցից հավասարահեռ
            u"\\textarmenian{Կոորդինատային ցանցի գծերը մնում են}",
            u"\\textarmenian{զուգահեռ}",
            u"\\textarmenian{և}",
            u"\\textarmenian{միմյանցից հավասարահեռ}",
        ])
...
class YetAnotherLinearTransformation(SimpleLinearTransformationScene):
    ...
    def construct(self):
        SimpleLinearTransformationScene.construct(self)
        words = TextMobject(u"""\\textarmenian{
            Ինչպե՞ս նկարագրենք սրանք թվաբանորեն}"""
        ) # [poxel]
        words.add_background_rectangle()
        words.to_edge(UP)
        ...
        # formula = TexMobject([
        #     matrix_to_tex_string([u"x_\\textarmenian{սկզբ}", u"y_\\textarmenian{սկզբ}"]),
        #     "\\rightarrow ???? \\rightarrow",
        #     matrix_to_tex_string(["x_\\text{out}", "y_{\\text{out}}"])
        # ])
        formula = TextMobject(u"\\left[ \\begin{array}{c}x_\\textarmenian{սկզբ} \\\\ y_\\textarmenian{սկզբ}\\end{array} \\right] \\rightarrow ???? \\rightarrow \\left[ \\begin{array}{c}x_\\textarmenian{ստաց} \\\\ y_\\textarmenian{ստաց}\\end{array} \\right]")
...
class TrackBasisVectorsExample(LinearTransformationScene):
    # sa problema, texyu unicode vichakova tpum
    CONFIG = {
    ...
    def label_bases(self):
        ...
        for vect, label, color in triplets:
            label_mobs.append(self.add_transformable_label(
                # vect, label, u"\\text{\\textarmenian{Ձևափոխված} } " + label,
                vect, label, u"\\textarmenian{Ձևափոխված} " + label,
    ...
    def write_linear_map_rule(self):
        # rule = TexMobject([
        #     # u"\\text{\\textarmenian{Ձևափոխված} } \\vec{\\textbf{v}}",
        #     u"\\textarmenian{Ձևափոխված} \\vec{\\textbf{v}}",
        #     " = %s"%self.v_coord_strings[0],
        #     # u"(\\text{\\textarmenian{Ձևափոխված} }\\hat{\\imath})",
        #     u"(\\textarmenian{Ձևափոխված} \\hat{\\imath})",
            
        #     "+%s"%self.v_coord_strings[1],
        #     # u"(\\text{\\textarmenian{Ձևափոխված} } \\hat{\\jmath})",
        #     u"(\\textarmenian{Ձևափոխված} \\hat{\\jmath})",
...
class NowWithoutWatching(Scene):
    def construct(self):
        text = TextMobject(u"\\textarmenian{Հիմա առանց նայելու}")
        text.to_edge(UP)
...
class MatrixVectorMultiplication(LinearTransformationScene):
    ...
    def label_matrix(self, matrix):
        title = TextMobject(u"\\textarmenian{«}2\\times2 \\textarmenian{ }\\textarmenian{մատրից»}")
        title.to_edge(UP+LEFT)
        col_circles = []
        for i, color in enumerate([X_COLOR, Y_COLOR]):
            col = VMobject(*matrix.get_mob_matrix()[:,i])
            col_circle = Circle(color = color)
            col_circle.stretch_to_fit_width(matrix.get_width()/2.5)
        ...
        i_message = TextMobject(u"\\textarmenian{$\\hat{\\imath}$-ն գնում է այստեղ}")
        j_message = TextMobject(u"\\textarmenian{$\\hat{\\jmath}$-ն գնում է այստեղ}")
    ...
    def multiply_by_vector(self, matrix):
        ...
        words = TextMobject(u"\\textarmenian{ինչ-որ մի վեկտոր}")
    ...
    def reposition_matrix_and_vector(self, matrix, vector, formula):
        ...
        brace_words = TextMobject(u"\\textarmenian{Ամբողջ ինտուիցիան այստեղ է}")
...
class ColumnsToBasisVectors(LinearTransformationScene):
    ...
    def move_matrix_columns(self, transposed_matrix, vector_coords = None):
        ...
        matrix_mob.to_corner(UP+LEFT).shift(0.73*RIGHT) # shift is added for likealrly dependent columns class
...
class Describe90DegreeRotation(LinearTransformationScene):
    CONFIG = {
        "transposed_matrix" : [[0.0, 1.0], [-1.0, 0.0]],
        "title" : u"\\textarmenian{$90^\\circ$ պտույտ ժամսլաքին հակառակ}",
    }
    def construct(self):
        self.setup()
        ...
        for vect, color, col in [(self.i_hat, X_COLOR, col1), (self.j_hat, Y_COLOR, col2)]:
            ...
            label.highlight(color).scale(0.3)
...
class DescribeShear(Describe90DegreeRotation):
    CONFIG = {
        "transposed_matrix" : [[1.0, 0.0], [1., 1.]],
        "title" : u"\\textarmenian{««Շեղում»»}",
    }

class OtherWayAround(Scene):
    def construct(self):
        self.play(Write(TextMobject(u"\\textarmenian{Իսկ ի՞նչ, եթե հակառակն անենք}")))
        self.dither(2)
...
class LinearlyDependentColumns(ColumnsToBasisVectors):
    def construct(self):
        self.setup()
        title = TextMobject(u"\\textarmenian{Սյուները գծորեն}")
        subtitle = TextMobject(u"\\textarmenian{կախված են}")
        title.add_background_rectangle()
...
class NextVideo(Scene):
    def construct(self):
        ...
        # custom_template.add_to_preamble(r"\usepackage[none]{hyphenat}")  # Disable hyphenation

        title = TextMobject(u"\\textarmenian{Հաջորդ տեսանյութը․ «Մատրիցների արտադրյալը որպես համադրություն»}", 
                            enforce_new_line_structure=False)
        title.scale_to_fit_width(16)
        # 4
        # title = TextMobject(u"\\textarmenian{Հաջորդ տեսանյութը․ \\\\«Գծային ձևափոխություններ եռաչափ տարածությունում»}").scale(0.9)
        title.to_edge(UP)
...
class FinalSlide(Scene):
    def construct(self):
        text = TextMobject("""
            \\footnotesize 
            Գծային, the definition of ``linear'' is as follows: 
